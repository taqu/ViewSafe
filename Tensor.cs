...
namespace ViewSafe
{
    ...
    public class Tensors
    {
		...
		public static Tensors? Open(string path)
		{
			try
			{
				return Tensors.JsonParser.parse(path);
			}
			catch (Exception e)
			{
		...
		private class JsonParser
        {
            ...
            public static Tensors? parse(string path)
            {
                if (!File.Exists(path))
                {
                    return null;
                ...
                if (fileInfo.Length < sizeof(ulong))
                {
                    return null;
                }
                ...
                fs.Read(data, 0, sizeof(long));
				long json_size = MemoryMarshal.Cast<byte, long>(data)[0];
                if (json_size <= 0 || (fileInfo.Length - sizeof(long)) < json_size)
                {
                    return null;
                }
                ...
                byte[] input = new byte[json_size];
                if(fs.Read(input, 0, (int)json_size)< json_size)
                {
					return null;
				}
